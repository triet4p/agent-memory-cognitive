...
Useful for verifying a Generation diagnosis: does the model really fail with
the current top-k context? Compare the output against the gold answer.

Usage:
    uv run python .claude/skills/cogmem-diagnose/scripts/test_generate.py \
        --bank-id COGMEM_EXP_v14_e567_c001 \
        --query "How many model kits have I worked on or bought?" \
        --gold-answer "5" \
        --top-k 25

    # Custom top-k to test if fewer results help
    uv run python .claude/skills/cogmem-diagnose/scripts/test_generate.py \
        --bank-id COGMEM_EXP_v14_e567_c001 \
        --query "How many model kits have I worked on or bought?" \
        --gold-answer "5" \
...
Output JSON:
    {
      ...
      "top_k_used": int,
      "model_answer": str,
      "recall_summary": [{rank, id, document_id, ce_score, text}]   # top-10 shown
    }
"""
import argparse
import json
...
def main():
    parser = argparse.ArgumentParser(description="Recall + generate to test the full pipeline.")
