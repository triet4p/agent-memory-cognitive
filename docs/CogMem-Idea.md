...
*LƯU Ý*: Có thể phải thu hẹp tập benchmark cho phù hợp tài nguyên.

### 2.3 HINDSIGHT — Baseline 

HINDSIGHT (Latimer et al., 2025) là hệ thống bộ nhớ tác nhân đạt: 91.4% trên LongMemEval với Gemini-3, 89.61% trên LoCoMo. Kiến trúc gồm 4 mạng (World, Experience, Opinion, Observation), pipeline Retain dùng LLM để trích xuất narrative facts theo kiểu **incremental** — mỗi session mới được processed qua: fact extraction → entity resolution → opinion reinforcement → observation regeneration → background merging. Pipeline Recall dùng 4-way parallel retrieval (semantic + BM25 + graph + temporal) kết hợp bằng Reciprocal Rank Fusion và cross-encoder reranking.
