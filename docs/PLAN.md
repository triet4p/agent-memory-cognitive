...
4. Sprint 1 - Atomic Task T1.2: thêm Lossless Metadata (raw_snippet) và mở rộng fact/network types habit-intention-action_effect + transition typing; output gồm migration mới ở cogmem_api/alembic/versions/, cập nhật cogmem_api/models.py, logs/task_102_summary.md, tests/artifacts/test_task102_schema_extensions.py; verification gồm Drift đối chiếu idea section 3.1-3.2 và giữ nguyên cột hạ tầng cũ, Behavioral test migration upgrade/downgrade + assert cột/enum mới, Isolation check chạy model import không dùng hindsight_api.
...
13. Sprint 4 - Reflect + consolidation thích nghi mạng mới. Atomic Task T4.1: fork reflect/consolidation tối thiểu cần thiết và mở rộng xử lý node mới; output gồm cogmem_api/engine/reflect/*, cogmem_api/engine/consolidation/* (phần dùng), logs/task_401_summary.md, tests/artifacts/test_task401_reflect_consolidation.py; verification gồm Drift bảo toàn stripped-down CARA behavior quan trọng, Behavioral test trả lời có thể dùng raw_snippet context sau retrieve, Isolation check công cụ reflect chỉ gọi cogmem_api engine.
...
- f:/ai-ml/agent-memory-cognitive/hindsight_api/engine/consolidation/consolidator.py - consolidation baseline
