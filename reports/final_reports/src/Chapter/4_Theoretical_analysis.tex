...
\textbf{Semantic memory} lưu tri thức ổn định và ít phụ thuộc episode. Trong CogMem, \textbf{World Network} giữ các fact dạng nghề nghiệp, kỹ năng, quan hệ khách quan và tri thức miền.
}}
...
\centering
\fbox{\parbox{0.86\textwidth}{
\textbf{Experience} biểu diễn một lần xảy ra; \textbf{Habit} biểu diễn pattern lặp lại. Liên kết S-R giúp nối habit với các experience làm bằng chứng cho pattern đó.
}}
...
\fbox{\parbox{0.86\textwidth}{
\centering
planning intention $\rightarrow$ fulfilled\_by $\rightarrow$ experience\\
experience $\rightarrow$ triggered $\rightarrow$ new intention\\
\small Abandoned là trạng thái của intention node, không phải edge type riêng.
}}
...
\centering
\fbox{\parbox{0.86\textwidth}{
\textbf{Theory of Event Coding} gắn hành động với hệ quả cảm nhận được. CogMem chuyển ý tưởng này thành node \textit{action\_effect}: precondition -- action -- outcome.
}}
...
\centering
\fbox{\parbox{0.86\textwidth}{
\textbf{Gist trace} tương ứng với fact text đã nén để retrieval nhanh. \textbf{Verbatim trace} tương ứng với raw\_snippet để giữ lại chi tiết nguyên văn cho synthesis.
}}
...
\centering
\fbox{\parbox{0.86\textwidth}{
\textbf{Episodic buffer} tích hợp nhiều nguồn tín hiệu thành một biểu diễn làm việc. Trong CogMem, SUM spreading activation đóng vai trò hợp lưu các evidence path rải rác.
}}
...
\centering
\fbox{\parbox{0.86\textwidth}{
\textbf{Attentional selection} ở cấp hệ thống: temporal queries ưu tiên temporal channel, causal queries ưu tiên graph/action-effect, preference queries ưu tiên habit/opinion, multi-hop queries ưu tiên graph traversal.
}}
