...
\textit{Tiếp nối bối cảnh bài toán từ Chương 1, chương này cung cấp nền tảng kiến thức cốt lõi về hệ thống nhận thức của não bộ và đồ thị tri thức, làm cơ sở để phân tích ưu nhược điểm của các hệ thống bộ nhớ tác nhân đương đại.}
...
\begin{itemize}
    \item \textbf{Giới hạn Cửa sổ Ngữ cảnh (Context Window Bottleneck) và Chi phí (O($N^2$))}: Cơ chế Self-Attention cốt lõi của kiến trúc Transformer đòi hỏi độ phức tạp tính toán và bộ nhớ tăng theo bình phương độ dài chuỗi đầu vào (O($N^2$)). Điều này khiến việc đưa toàn bộ lịch sử hội thoại dài hạn vào prompt (context stuffing) trở nên bất khả thi về mặt tài nguyên phần cứng và làm tăng độ trễ nội tại.
...
Để giải quyết các rào cản này, sự chuyển dịch từ việc phụ thuộc vào \textit{Context Window} sang các hệ thống \textit{Bộ nhớ Dài hạn bên ngoài (External Long-term Memory)} là xu thế bắt buộc.
...
\vspace{1em}
\noindent\textit{Bằng việc hệ thống hóa các giới hạn toán học và cấu trúc của các mô hình hiện hành, Chương 3 tiếp theo sẽ chính thức đề xuất kiến trúc CogMem — một giải pháp thiết kế bộ nhớ toàn diện lấy cảm hứng trực tiếp từ sinh học để giải quyết triệt để các rào cản đó.}
